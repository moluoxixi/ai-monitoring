...
清理仅针对应用源码树中的空壳目录。删除 `apps/server/src/answer-summary/dto`、`apps/server/src/platforms/dto`、`apps/web/src/components/platforms`，并级联删除随之暴露且同样无引用的 `apps/server/src/answer-summary`、`apps/server/src/platforms`；`data/openclaw-outbound`、`.moluoxixi/tasks/archive` 以及 `target`、`.venv`、`node_modules` 下的空目录分别由运行时、任务归档或工具链负责，不作为源码垃圾处理。
