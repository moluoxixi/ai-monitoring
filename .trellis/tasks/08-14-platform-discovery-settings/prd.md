...
- R7. 用户可以持久化选择启用的平台；选择结果同时影响扩展页、消息概览和后续通知投递，但不删除或阻止事件采集入库。
...
- R15. Windows 与 macOS 均支持宿主平台扫描；界面显示当前操作系统及容器状态，CLI 与 Desktop 必须保持独立识别。
...
- [x] AC13. macOS 扫描使用原生命令和应用包路径，不调用 Windows 探针；界面展示当前设备。
- [x] AC14. 未启用平台的新事件仍入库，但不产生通知投递且不出现在消息概览；重新启用后历史事件可再次显示。
